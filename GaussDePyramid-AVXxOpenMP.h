...
#ifndef SIFT_GUASS_NORMAL_GAUSSDEPYRAMID_AVXXOPENMP_H
#define SIFT_GUASS_NORMAL_GAUSSDEPYRAMID_AVXXOPENMP_H
...
class GaussPyramid_aomp {
public:
    int **data; //记录图像灰度数据
    GaussPyramid_aomp();

    GaussPyramid_aomp(int **img, int len/*,int wid*/, int S);
...
GaussPyramid_aomp::GaussPyramid_aomp() {
    data = nullptr;
}

GaussPyramid_aomp::GaussPyramid_aomp(int **img, int len, int S) {
    length = len;
...
//初始化高斯金字塔，尚未进行高斯滤波操作
void GaussPyramid_aomp::GaussPyInit() {
    int step = 1;
    if(!initialized)
...
void GaussPyramid_aomp::GaussFilter(int theLayer) {//采用双边滤波
//    float len = length;
//    int t = theLayer;
//    while (theLayer != 0) {
//        theLayer--;
...
void GaussPyramid_aomp::GenerateDoG() {
    int len = length;
    for (int i = 0; i < layer; ++i) {
        GaussFilter(i);
        for (int j = 0; j < S + 2; ++j) {
...
void GaussPyramid_aomp::GenerateDoG_nomp_dynamic() {
    float fil[length];
    for (int i = 0; i < S; i++) {
        int len = length;
        int k = i;
...
//                            float32x4_t vf, va;
                            vf = _mm256_loadu_ps(fil + n);
                            va = _mm256_loadu_ps(GaussPy[j][i][m] + n);
                            va = _mm256_mul_ps(va,vf);
                            _mm256_store_ps(GaussPy[j][i][m] + n, va);
//                            GaussPy[j][i][m][n] *= fil[n];
                        }
                    }
#pragma omp for
...
//                    __m256 vb,va;
                    for (int j = 0; j < len; ++j) {
                        for (int l = 0; l < len; l += 8) {
                            va = _mm256_loadu_ps(GaussPy[MyLayer][i][j] + l);
                            vb = _mm256_loadu_ps(GaussPy[MyLayer][i + 1][j] + l);
                            va = _mm256_sub_ps(va, vb);
                            _mm256_store_ps(GaussPy[MyLayer][i][j] + l, va);
//                        GaussPy[MyLayer][i][j][l] -= GaussPy[MyLayer][i + 1][j][l];
                        }
                    }
                }
...
void GaussPyramid_aomp::GenerateDoG_nomp_static() {
...
#endif //SIFT_GUASS_NORMAL_GAUSSDEPYRAMID_AVXXOPENMP_H
